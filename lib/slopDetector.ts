...
function calculateRepetitiveness(
  words: string[],
  sentences: string[],
  details: string[]
): number {
  ...
  if (highRepeatWords.length > 0) {
    score += 30;
    details.push(
      `Highly repeated words: ${highRepeatWords.map(([w, c]) => `"${w}" (${c}x)`).join(', ')}`
    );
  }
  ...
  const repeatedSentences = Array.from(sentenceFreq.entries()).filter(
    ([, count]) => count > 1
  );
  if (repeatedSentences.length > 0) {
    score += 40;
    details.push(
      `${repeatedSentences.length} sentence(s) repeated multiple times`
    );
  }
...
function calculateAIScore(lowerContent: string, details: string[]): number {
  ...
  if (foundPhrases.length > 0) {
    details.push(
      `AI-typical phrases detected: ${foundPhrases.slice(0, 3).join(', ')}${foundPhrases.length > 3 ? '...' : ''}`
    );
  }
...
function calculateClickbaitScore(
  lowerContent: string,
  content: string,
  details: string[]
): number {
...
function calculateLowEffortScore(
  content: string,
  words: string[],
  sentences: string[],
  details: string[]
  ...
  if (avgSentenceLength > 50) {
    score += 30;
    details.push(
      `Unusually long sentences (avg ${Math.round(avgSentenceLength)} words)`
    );
  }
